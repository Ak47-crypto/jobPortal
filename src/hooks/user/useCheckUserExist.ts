...
function useCheckUserExist() {
    const {setUserData}=useAppContext()
     const checkUserProviderExist = async (
        user: Array<string>
        ...
        //   localStorage.setItem('userData',JSON.stringify(userReturn))
          setUserData(userReturn)
       ...
       const checkUserWorkerExist = async (
        user: Array<string>
      ): Promise<returnObj | errorObj> => {
        const isProviderExist = false;
        const isWorkerExist = false;
  ...
  return {checkUserProviderExist,checkUserWorkerExist}
}
